...
| 标题生成 | 选中 N 个批次 → 各取第 K 张已完成图 → 多模态模型（gemini-3.7-flash / grok-4.5 / gpt-5.6-sol）生成电商标题，支持重新生成；CSV 导出（即将推出） |
...
项目根目录提供 `start-dev.bat`（Windows 双击即用），自动完成：

1. **依赖检测**：首次运行自动执行 `uv sync`（后端）与 `npm install`（前端），之后跳过
2. **双窗口启动**：分别打开"后端 :8000"与"前端 :5173"两个窗口，各自独立日志，关闭窗口即停止对应服务

> 首次使用请先 `cp .env.example .env` 并填入 `TOAPIS_API_KEY`；新机器若使用 MySQL，
> 需先按第 4 节初始化数据库（表结构缺失的列会在后端启动时自动补齐，无需手动 ALTER）。
